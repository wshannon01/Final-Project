...
crime.sum.yr <- function(df15, df16, x){
  ...
  p <- ggplot(data = m.df, aes(x=Var2, y=n, group = factor(year), color = factor(year))) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + stat_smooth(se = FALSE)
  return(p)
  ...
  p <- ggplot(data = m.df, aes(x=Var2, y=n, color = factor(year))) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + stat_smooth((aes(group = 2)),se = FALSE)
...
crime.sum.yr(dat.15, dat.16, "month")
